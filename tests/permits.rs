...
use cfrm::{
    ...
    permits::{Permit, PermitEpoch, PreparedPermit, RecipientClaim, ISSUANCE_REQUEST_BYTES},
    Error,
};
...
fn stamp_key() -> SigningKey {
    SigningKey::from_bytes(&[71; 32])
}
fn epoch() -> PermitEpoch {
    PermitEpoch {
        community_id: "community.example".into(),
        epoch_id: "test-epoch".into(),
        valid_from: 100,
        issue_until: 400,
        expires_at: 500,
        public_key_der: BASE64URL_NOPAD.encode(
...
fn request(f: &Fixture, key: &SigningKey, payload: &[u8], nonce: u8) -> AllocationRequest {
    let mut request = AllocationRequest {
        community_id: f.trust.community_id.clone(),
        member_id: member_id(5),
        chat_public_key: BASE64URL_NOPAD.encode(&key.verifying_key().to_bytes()),
        policy_digest: policy_digest(&policy()).unwrap(),
        nonce: encoded(nonce),
        blinded_request: BASE64URL_NOPAD.encode(payload),
        issued_at: 110,
        expires_at: 150,
    ...
    let bytes = serde_json::to_vec(&serde_json::json!([
        ...
        request.nonce,
        BASE64URL_NOPAD.encode(&Sha256::digest(payload)),
        request.issued_at,
        request.expires_at
    ]))
    .unwrap();
    request.signature = BASE64URL_NOPAD.encode(&key.sign(&bytes).to_bytes());
    request
}
fn issue_permit(f: &Fixture, ledger: &mut AllocationLedger, nonce: u8) -> Permit {
    ...
    let response = PermitIssuer::from_pkcs1_der(&epoch, key_der())
        .unwrap()
        .issue(
            ledger,
            &f.grant(5, &f.device),
...
fn real_blind_issue_debit_recovery_and_private_recipient_redemption() {
    ...
    assert!(!wire.contains(&member_id(5)));
    assert!(!wire.contains(&member_id(6)));
    let path = dir.path().join("spent.sqlite");
    let stamp = {
        let mut redeemer = PermitRedeemer::open(&path, &epoch, stamp_key()).unwrap();
        redeemer.redeem(&claim.request, || 160).unwrap()
    };
    let mut redeemer = PermitRedeemer::open(&path, &epoch, stamp_key()).unwrap();
    ...
    substituted.binding.recipient_id = member_id(7);
    assert_eq!(
        substituted.verify_stamp(&epoch, &stamp, 161),
        Err(Error::Admission)
    );
    let mut changed_challenge = claim.clone();
    changed_challenge.binding.challenge = encoded(9);
    assert_eq!(
        changed_challenge.verify_stamp(&epoch, &stamp, 161),
        Err(Error::Admission)
    );
    let mut changed_intro = claim.clone();
    changed_intro.binding.introduction_id = encoded(9);
    assert_eq!(
        changed_intro.verify_stamp(&epoch, &stamp, 161),
        Err(Error::Admission)
    );
    let mut forged_stamp = stamp.clone();
    forged_stamp.signature = BASE64URL_NOPAD.encode(&[1; 64]);
    assert_eq!(
        claim.verify_stamp(&epoch, &forged_stamp, 161),
        Err(Error::Signature)
    );
}
...
fn simultaneous_devices_share_allowance_and_invalid_rsa_input_never_debits() {
    ...
    let path = dir.path().join("allowance.sqlite");
    let mut ledger = AllocationLedger::open(&path, f.trust.clone(), policy()).unwrap();
    let mut invalid = vec![0; ISSUANCE_REQUEST_BYTES];
    invalid[..32].copy_from_slice(
        &BASE64URL_NOPAD
            .decode(epoch.context_id().unwrap().as_bytes())
            .unwrap(),
    );
    let issuer = PermitIssuer::from_pkcs1_der(&epoch, key_der()).unwrap();
    assert!(issuer
    ...
    for seed in [10, 11, 12] {
        ...
        workers.push(std::thread::spawn(move || {
            ...
            let prepared = PreparedPermit::new(&epoch, 110).unwrap();
            let request = request(&f, &key, &prepared.issuance_request(), seed);
            let issuer = PermitIssuer::from_pkcs1_der(&epoch, key_der()).unwrap();
            ready.wait();
            let response = issuer.issue(
                &mut ledger,
                &f.grant(5, &key),
                &f.authorize(5, &key),
                &request,
    ...
    assert_eq!(
        results
            .iter()
            .filter(|result| matches!(result, Err(Error::NoAllowance)))
            .count(),
        1
    );
...
fn copied_bearer_permit_can_win_only_one_recipient_claim_even_concurrently() {
    ...
    for claim in [first, second] {
        ...
        workers.push(std::thread::spawn(move || {
            ready.wait();
            redeemer.redeem(&claim.request, || 120)
        }));
    }
    let results: Vec<_> = workers
        .into_iter()
    ...
    assert_eq!(
        results
            .iter()
            .filter(|result| matches!(result, Err(Error::Replay)))
            .count(),
        1
    );
}
...
fn wrong_context_mutation_common_expiry_and_key_reuse_are_rejected() {
    ...
    other.epoch_id = "other-epoch".into();
    ...
    assert_eq!(
        redeemer.redeem(&claim.request, || 119),

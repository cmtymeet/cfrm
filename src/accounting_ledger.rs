...
use crate::{
    accounting::{account_acceptance_bytes, account_request_bytes, account_request_digest,
        account_status_bytes, account_status_response_bytes, field, verify_account_acceptance, AccountAcceptance,
        AccountPolicy, AccountProofScope, AccountProofVerifier, AccountRequest,
        AccountStatusRequest, AccountStatusResponse},
    admission::{decode, signature, verify_admission, verify_device_authorization,
        AdmissionGrant, AdmissionTrust, DeviceAuthorization, MAX_INTEGER},
...
fn advance_clock(transaction: &Transaction<'_>, now: u64) -> Result<(), Error> {
    transaction.execute("UPDATE cfrm_accounts_config SET clock_floor=?1 WHERE singleton=1",
        [sql_integer(now)?])?;
    Ok(())
}

fn cached(transaction: &Transaction<'_>, owner: &[u8; 32], request: &[u8; 32],
    operator_key: &[u8; 32]) -> Result<Option<AccountAcceptance>, Error>
{
    let result: Option<Vec<u8>> = transaction.query_row(
        "SELECT acceptance FROM cfrm_accounts_requests WHERE owner=?1 AND request_id=?2",
        params![owner.as_slice(), request.as_slice()], |row| row.get(0)).optional()?;
    result.map(|bytes| {
        let result: AccountAcceptance = serde_json::from_slice(&bytes).map_err(|_| Error::Storage)?;
...
impl<V: AccountProofVerifier> AccountLedger<V> {
    pub fn open(path: impl AsRef<Path>, trust: AdmissionTrust, policy: AccountLedgerPolicy,
        verifier: V, operator: SigningKey) -> Result<Self, Error>
    {
        if !crate::admission::scope(&trust.community_id)
            ...
            || policy.max_proof_bytes == 0 || policy.max_proof_bytes > i32::MAX as usize
        ...
        let config = serde_json::to_vec(&serde_json::json!([
            "cfrm.account.database.v1", trust.community_id, trust.policy_digest,
            trust.issuer_public_key, policy, proof_scope, operator.verifying_key().to_bytes()
        ...
        let prior: Option<Vec<u8>> = transaction.query_row(
        ...
        transaction.commit()?;
        Ok(Self { connection, trust, community, policy, policy_digest, proof_scope, verifier, operator })
    }
    ...
    pub fn admit_checkpoint(&mut self, slot: u64, root: [u8; 32]) -> Result<(), Error> {
        field(&root, true)?;
        let end = slot.checked_add(1).and_then(|n| n.checked_mul(self.policy.checkpoint_period_seconds))
            .filter(|end| *end <= MAX_INTEGER).ok_or(Error::InvalidInput)?;
        if end == 0 { return Err(Error::InvalidInput); }
        let transaction = self.connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let prior: Option<Vec<u8>> = transaction.query_row(
            "SELECT root FROM cfrm_accounts_checkpoints WHERE slot=?1", [sql_integer(slot)?],
            |row| row.get(0)).optional()?;
        if let Some(prior) = prior {
            if prior.as_slice() != root { return Err(Error::PolicyMismatch); }
        } else {
            transaction.execute("INSERT INTO cfrm_accounts_checkpoints VALUES(?1,?2)",
    ...
    pub fn apply(&mut self, grant: &AdmissionGrant, authorization: &DeviceAuthorization,
        request: &AccountRequest, clock: impl Fn() -> u64) -> Result<AccountAcceptance, Error>
    {
        if request.proof.len() > self.policy.max_proof_bytes { return Err(Error::InvalidInput); }
        let statement = &request.statement;
        if statement.community != self.community || statement.policy != self.policy.account
            || statement.policy_digest != self.policy_digest || request.proof_scope != self.proof_scope
        ...
        signature(&request.chat_public_key, &account_request_bytes(request)?, &request.signature)?;
        ...
        if let Some(result) = cached(&transaction, &statement.owner, &request.request_id, &operator_key)? {
            if result.request_digest != request_digest { return Err(Error::Replay); }
            advance_clock(&transaction, now)?;
            transaction.commit()?;
            return Ok(result);
        }
        if request.expires_at - request.issued_at > self.policy.max_authorization_seconds
            || now < request.issued_at || now >= request.expires_at
            || request.expires_at > grant.expires_at || request.expires_at > authorization.expires_at
        ...
        if now / self.policy.checkpoint_period_seconds != slot { return Err(Error::Expired); }
        let checkpoint: Option<Vec<u8>> = transaction.query_row(
            "SELECT root FROM cfrm_accounts_checkpoints WHERE slot=?1", [sql_integer(slot)?],
            |row| row.get(0)).optional()?;
        if checkpoint.as_deref() != Some(statement.enrollment_root.as_slice()) {
            return Err(Error::Admission);
        }
        let prior: Option<(Vec<u8>, i64, Vec<u8>)> = transaction.query_row(
            "SELECT root_key,version,state FROM cfrm_accounts WHERE owner=?1",
            [statement.owner.as_slice()], |row| Ok((row.get(0)?,row.get(1)?,row.get(2)?))).optional()?;
        match prior {
            None if statement.genesis => {},
            Some((key, version, state)) if !statement.genesis
        ...
        if statement.settlement_marker != [0; 32] {
            let exists: bool = transaction.query_row(
                ...
                params![statement.owner.as_slice(), statement.settlement_marker.as_slice()], |row| row.get(0))?;
            if exists { return Err(Error::Replay); }
        }
        self.verifier.verify(statement, &request.proof)?;
        let completed = clock();
        check_time(completed, now)?;
        if completed >= request.expires_at || completed >= grant.expires_at
        ...
        let mut acceptance = AccountAcceptance {
            statement: statement.clone(), request_id: request.request_id, request_digest,
            proof_scope: self.proof_scope.clone(), accepted_at: completed, signature: String::new(),
        };
        acceptance.signature = BASE64URL_NOPAD.encode(&self.operator.sign(
            &account_acceptance_bytes(&acceptance)?).to_bytes());
        let response = serde_json::to_vec(&acceptance).map_err(|_| Error::Storage)?;
        if statement.genesis {
            transaction.execute("INSERT INTO cfrm_accounts VALUES(?1,?2,?3,?4,?5)",
                params![statement.owner.as_slice(), root_key.as_slice(), sql_integer(statement.next_version)?,
                    statement.next_state.as_slice(), request.request_id.as_slice()])?;
        } else {
            ...
            if statement.settlement_marker != [0; 32] {
                transaction.execute("INSERT INTO cfrm_accounts_markers VALUES(?1,?2)",
            ...
            if changed != 1 { return Err(Error::Replay); }
        }
        transaction.execute("INSERT INTO cfrm_accounts_requests VALUES(?1,?2,?3)",
            params![statement.owner.as_slice(), request.request_id.as_slice(), response])?;
    ...
    pub fn status(&mut self, grant: &AdmissionGrant, authorization: &DeviceAuthorization,
        request: &AccountStatusRequest, clock: impl Fn() -> u64) -> Result<AccountStatusResponse, Error>
    {
        let bytes = account_status_bytes(request)?;
        if request.community != self.community || decode::<32>(&grant.member_id)? != request.owner
            || request.chat_public_key != grant.chat_public_key
        { return Err(Error::Admission); }
        if request.expires_at - request.issued_at > self.policy.max_authorization_seconds {
            return Err(Error::Expired);
        ...
        let request_id = match request.request_id {
            Some(id) => Some(id),
            None => {
                let value: Option<Vec<u8>> = transaction.query_row(
                    "SELECT latest_request FROM cfrm_accounts WHERE owner=?1", [request.owner.as_slice()],
                    |row| row.get(0)).optional()?;
                value.map(|value| value.try_into().map_err(|_| Error::Storage)).transpose()?
            }
        };
        let result = request_id.map(|id| cached(&transaction, &request.owner, &id,
            &self.operator.verifying_key().to_bytes())).transpose()?.flatten();
        ...
        let mut response = AccountStatusResponse {
            status_request_digest: Sha256::digest(signed_request).into(), observed_at: now,
            acceptance: result, signature: String::new(),
        };
        response.signature = BASE64URL_NOPAD.encode(&self.operator.sign(
            &account_status_response_bytes(&response)?).to_bytes());
        advance_clock(&transaction, now)?;
        transaction.commit()?;
        Ok(response)
    }

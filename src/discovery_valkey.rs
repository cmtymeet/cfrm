...
impl ValkeyDiscoveryStore {
    pub fn connect(config: ValkeyConfig, control: Arc<SqliteDiscoveryControl>) -> Result<Self, Error> {
        ...
        Ok(Self { pool, namespace: config.namespace, pool_timeout: config.pool_timeout, control })
    }
    ...
    fn fetch(&self, connection: &mut redis::Connection, prefix: &str, community: &str, member: &str, now: u64)
        ...
        let Some(state) = self.control.current(community, member, now)? else { return Ok(None); };
        let hash = digest(&serde_json::to_vec(&publication).map_err(|_| Error::Storage)?);
        let until = state.live_until(now);
        if state.publication_hash != hash || until <= now { return Ok(None); }
...
impl DiscoveryStore for ValkeyDiscoveryStore {
    fn execute(&self, request: &VerifiedDiscoveryRequest, limits: &DiscoveryLimits, now: u64)
        ...
        if !matches!(request.operation, DiscoveryOperation::Query { .. } | DiscoveryOperation::Fetch { .. }) {
            self.control.with_current(&commit, |state| {
                let body = commit.publication.as_ref().map(serde_json::to_string).transpose()
                    .map_err(|_| Error::InvalidInput)?.unwrap_or_default();
                let result: i32 = redis::Script::new(COMMIT).key(&profile_key).key(&catalog_key).key(&live_key)
                    .arg(&request.member_id).arg(state.live_until(now)).arg(now).arg(body)
                    .arg(state.retain_until.saturating_sub(now).max(1))
                    .invoke(&mut *connection).map_err(|_| Error::Storage)?;
                if result != 1 { return Err(Error::Storage); }
                Ok(())
            })?;
        }
        match &request.operation {
            DiscoveryOperation::Fetch { member_id } => Ok(DiscoveryResponse::Profile {
                publication: self.fetch(&mut connection, &prefix, &request.community_id, member_id, now)?.map(|(value, _)| value),
            }),

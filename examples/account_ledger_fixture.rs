...
use cfrm::{accounting::*, accounting_ledger::*, admission::{AdmissionGrant, AdmissionTrust, DeviceAuthorization}, Error};
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct Checkpoint { slot: u64, root: [u8;32] }
...
struct RealProcessVerifier { scope: AccountProofScope, script: PathBuf, manifest: PathBuf, enrollment: PathBuf }
impl AccountProofVerifier for RealProcessVerifier {
    fn scope(&self) -> AccountProofScope { self.scope.clone() }
    fn verify(&self, statement: &AccountStatement, proof: &[u8]) -> Result<(), Error> {
        let bytes = serde_json::to_vec(&json!({ "statement": statement,
            "proof": HEXLOWER.encode(proof), "proofScope": self.scope })).map_err(|_| Error::CryptoProvider)?;
        ...
        child.stdin.take().ok_or(Error::CryptoProvider)?.write_all(&bytes).map_err(|_| Error::CryptoProvider)?;
        let output = child.wait_with_output().map_err(|_| Error::CryptoProvider)?;
        if !output.status.success() || output.stdout.len() > 4096 { return Err(Error::CryptoProvider); }
        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase", deny_unknown_fields)]
        struct Verdict { verified: bool, proof_scope: AccountProofScope }
        let verdict: Verdict = serde_json::from_slice(&output.stdout).map_err(|_| Error::CryptoProvider)?;
        if !verdict.verified || verdict.proof_scope != self.scope { return Err(Error::CryptoProvider); }
        Ok(())
    }
}
...
fn run() -> Result<Value, Box<dyn std::error::Error>> {
    ...
    if args.len() != 7 && args.len() != 8 { return Err("config database verifier manifest enrollment mode [drop-response] required".into()); }
    ...
    if input.len() > 2_000_000 { return Err("fixture input bound".into()); }
    ...
    for checkpoint in config.checkpoints { ledger.admit_checkpoint(checkpoint.slot, checkpoint.root)?; }
...
fn main() {
    match run() {
        ...
        Err(error) => { println!("{}", json!({ "ok": false, "error": error.to_string() })); std::process::exit(1); }
    }
}

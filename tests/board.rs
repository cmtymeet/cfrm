...
use common::{encoded, onion, Fixture};
...
fn update(f: &Fixture, key: &SigningKey, member: u8, sequence: u64, endpoint: bool) -> PresenceUpdate {
    let mut value = PresenceUpdate { community_id: f.trust.community_id.clone(), member_id: encoded(member), chat_public_key: BASE64URL_NOPAD.encode(&key.verifying_key().to_bytes()), sequence, issued_at: 110, expires_at: 150, endpoint: endpoint.then(|| OnionEndpoint { host: onion(), port: 443 }), signature: String::new() };
...
fn one_member_has_multiple_devices_and_one_identical_public_snapshot() {
    ...
    board.apply(&f.grant(5,&f.device), &update(&f,&f.device,5,1,true), 110).unwrap();
    board.apply(&f.grant(5,&second), &update(&f,&second,5,1,true), 110).unwrap();
...
fn device_signature_binds_member_route_and_entire_update() {
    ...
    assert_eq!(board.apply(&grant,&forged,110), Err(Error::Signature));
    let mut wrong = update(&f,&f.device,6,1,true); sign(&mut wrong,&f.device);
    assert_eq!(board.apply(&grant,&wrong,110), Err(Error::Admission));
...
fn bounds_devices_members_time_and_validates_onions_before_publication() {
    ...
    assert_eq!(board.apply(&grant,&bad,110),Err(Error::InvalidInput));
    let valid=update(&f,&f.device,5,1,true); board.apply(&grant,&valid,110).unwrap();
    assert_eq!(board.snapshot(109),Err(Error::ClockRollback));
    for seed in [7,8] { let key=SigningKey::from_bytes(&[seed;32]); let result=board.apply(&f.grant(5,&key),&update(&f,&key,5,1,true),111); assert_eq!(result, if seed==7 {Ok(())} else {Err(Error::Capacity)}); }
}

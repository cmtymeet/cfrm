...
use crate::{
    ...
    discovery::{CachedProfile, DiscoveryLimits, DiscoveryOperation, DiscoveryResponse,
        DiscoveryStore, DiscoverySummary, VerifiedDiscoveryRequest},
    Error,
};
...
impl Counter {
    fn take(&mut self, now: u64, window: u64, maximum: u64) -> Result<(), Error> {
        ...
        if self.start != start { self.start = start; self.used = 0; }
        if self.used >= maximum { return Err(Error::Capacity); }
        self.used += 1;
        Ok(())
    }
}
...
impl MemberControl {
    pub(crate) fn live_until(&self, now: u64) -> u64 {
        self.leases.values().map(|v| v.expires_at).filter(|v| *v > now)
            .max().unwrap_or(0).min(self.profile_expires_at)
    }
}
...
pub(crate) fn transition(mut state: MemberControl, request: &VerifiedDiscoveryRequest,
    limits: &DiscoveryLimits, now: u64) -> Result<Change, Error> {
    if now < state.last_now { return Err(Error::ClockRollback); }
    if request.issued_at > now || now >= request.expires_at { return Err(Error::Expired); }
    state.last_now = now;
    state.replay.retain(|_, marker| marker.expires_at > now);
    state.leases.retain(|_, lease| lease.retain_until > now);
    let marker = digest(&serde_json::to_vec(&(
        "cfrm.discovery.replay.v1", &request.community_id, &request.member_id,
        &request.session_id, &request.request_id,
    )).map_err(|_| Error::InvalidInput)?);
    let is_write = !matches!(request.operation, DiscoveryOperation::Query { .. } | DiscoveryOperation::Fetch { .. });
    let write_digest = if is_write { Some(digest(&serde_json::to_vec(&(
        &request.chat_public_key, request.issued_at, request.expires_at, &request.operation,
    )).map_err(|_| Error::InvalidInput)?)) } else { None };
    if let Some(previous) = state.replay.get(&marker) {
        if write_digest.is_some() && previous.write_digest == write_digest {
            return Ok(Change { control: state, publication: None, retry: true });
        }
        return Err(Error::Replay);
    }
    if state.replay.len() >= limits.max_replay_entries { return Err(Error::Capacity); }
    state.replay.insert(marker, ReplayMarker { expires_at: request.expires_at, write_digest });
    let longest_window = limits.publish_window_seconds.max(limits.discriminator_window_seconds)
        .max(limits.read_window_seconds).max(limits.max_request_seconds).max(limits.max_lease_seconds);
    state.retain_until = state.retain_until.max(now.checked_add(longest_window).ok_or(Error::InvalidInput)?);
    let mut publication = None;
    match &request.operation {
        DiscoveryOperation::Publish { publication: value, lease } => {
        ...
        DiscoveryOperation::Disconnect { lease_id, sequence } => {
            let previous = state.leases.get_mut(&request.chat_public_key).ok_or(Error::Replay)?;
            if previous.lease_id != *lease_id || *sequence <= previous.sequence { return Err(Error::Replay); }
            previous.sequence = *sequence;
            previous.expires_at = now;
            previous.retain_until = previous.retain_until.max(now.checked_add(limits.max_request_seconds)
                .ok_or(Error::InvalidInput)?);
        }
        DiscoveryOperation::Query { .. } | DiscoveryOperation::Fetch { .. } => {
            state.read.take(now, limits.read_window_seconds, limits.read_limit)?;
        }
    }
    Ok(Change { control: state, publication, retry: false })
}

fn update_lease(state: &mut MemberControl, request: &VerifiedDiscoveryRequest,
    lease: &crate::discovery::DiscoveryLease, limits: &DiscoveryLimits, now: u64) -> Result<(), Error> {
    if let Some(previous) = state.leases.get(&request.chat_public_key) {
        if lease.sequence <= previous.sequence { return Err(Error::Replay); }
        // A different session may replace a device, but cannot replay its older lease.
    } else if state.leases.len() >= limits.max_devices_per_member { return Err(Error::Capacity); }
    if lease.expires_at <= now { return Err(Error::Expired); }
    let retain_until = lease.expires_at.max(now.checked_add(limits.max_request_seconds)
        .ok_or(Error::InvalidInput)?);
    state.leases.insert(request.chat_public_key.clone(), LeaseState {
        lease_id: lease.lease_id.clone(), sequence: lease.sequence, expires_at: lease.expires_at, retain_until,
    });
...
pub(crate) fn summary(publication: &CachedProfile, expires_at: u64) -> DiscoverySummary {
    ...
    DiscoverySummary { member_id: envelope.member_id.clone(), chat_public_key: envelope.chat_public_key.clone(),
        profile_digest: envelope.profile_digest.clone(), sequence: envelope.sequence,
...
impl DiscoveryStore for MemoryDiscoveryStore {
    fn execute(&self, request: &VerifiedDiscoveryRequest, limits: &DiscoveryLimits, now: u64)
        ...
        if now < community.last_now { return Err(Error::ClockRollback); }
        community.last_now = now;
        community.controls.retain(|_, value| value.retain_until > now);
        community.profiles.retain(|_, (_, until)| *until > now);
        if !community.controls.contains_key(&request.member_id) && community.controls.len() >= limits.max_members {
            return Err(Error::Capacity);
        }
        let change = transition(community.controls.get(&request.member_id).cloned().unwrap_or_default(),
            request, limits, now)?;
        if change.retry { return Ok(DiscoveryResponse::Updated); }
        let live_until = change.control.live_until(now);
        let previous = community.profiles.get(&request.member_id).map(|(value, _)| value.clone());
        let publication = change.publication.or(previous);
        if let Some(publication) = publication {
            if live_until > now { community.profiles.insert(request.member_id.clone(), (publication, live_until)); }
            else { community.profiles.remove(&request.member_id); }
        }

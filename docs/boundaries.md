...
Private reciprocity needs authenticated distinct participants, first-contact uniqueness, consent, hidden state transitions and replay protection. The real blind-permit experiment addresses finite allowance spending without sending a recipient ID to the issuer; it does not prove recipient consent, private counter correctness, noncollusion or absence of timing correlation. The separate [Semaphore acknowledgement experiment](../studies/private-acknowledgements.md) validates private eligible-member endorsements, lifetime nullifiers and a sender-excluded set. Its enrollment wrapper now binds one immutable Semaphore commitment to a certified stable member through two real signatures. Current qualification renewal, authenticated complete checkpoint publication and native client integration remain work. It does not prove delivery or sincere interaction. No complete production cryptographic accounting backend is implemented in the simulator.

...
impl PermitIssuer {
    ...
    pub fn from_pkcs1_der(epoch: &PermitEpoch, private_der: &[u8]) -> Result<Self, Error> {
        ...
        if BASE64URL_NOPAD.encode(&der) != epoch.public_key_der {
    ...
    fn blind_sign(&self, blinded: &[u8]) -> Result<Vec<u8>, Error> {
        let modulus = self
            ...
            .n()
            .to_vec();
        if blinded.len() != RSA_BYTES
            || blinded.iter().all(|byte| *byte == 0)
            || blinded >= modulus.as_slice()
        {
            return Err(Error::InvalidInput);
        }
        ...
        context
            .set_rsa_padding(Padding::NONE)
            .map_err(|_| Error::CryptoProvider)?;
        let mut result = Vec::new();
        context
            .sign_to_vec(blinded, &mut result)
            .map_err(|_| Error::CryptoProvider)?;
        if result.len() != RSA_BYTES {
    ...
    pub fn issue(
        ...
        if request.blinded_request.len() > ISSUANCE_REQUEST_BYTES * 2 {
            return Err(Error::InvalidInput);
        }
        let envelope = BASE64URL_NOPAD
            .decode(request.blinded_request.as_bytes())
            .map_err(|_| Error::InvalidInput)?;
        if envelope.len() != ISSUANCE_REQUEST_BYTES || envelope[..32] != decode::<32>(&context)? {
            return Err(Error::Admission);
        }
        let key_digest = digest(
            &BASE64URL_NOPAD
                .decode(self.epoch.public_key_der.as_bytes())
                .map_err(|_| Error::InvalidInput)?,
        );
...
impl PermitRedeemer {
    pub fn open(
        ...
        let existing: Option<String> = transaction
            .query_row(
                "SELECT context FROM cfrm_redemption_config WHERE singleton=1",
                [],
                |row| row.get(0),
            )
            .optional()?;
        match existing {
            ...
            None => {
                transaction.execute("INSERT INTO cfrm_redemption_config(singleton,context,clock_floor) VALUES(1,?1,0)", [&context])?;
            }
            _ => {}
        }
        transaction.commit()?;
    ...
    pub fn redeem(
        ...
        decode::<32>(&request.claim)?;
        decode::<32>(&request.commitment)?;
        ...
        if now > MAX_INTEGER || now < before || now < floor {
            return Err(Error::ClockRollback);
        }
        self.epoch.check_time(now, false)?;
        let prior: Option<(String, String, Vec<u8>)> = transaction
            .query_row(
                "SELECT claim,commitment,stamp FROM cfrm_redemptions WHERE token=?1",
                [&token],
        ...
        let stamp = match prior {
            Some((claim, commitment, bytes)) => {
                if claim != request.claim || commitment != request.commitment {
                    return Err(Error::Replay);
                }
                let stamp: RedemptionStamp =
                    serde_json::from_slice(&bytes).map_err(|_| Error::Storage)?;
                if stamp.context_id != self.epoch.context_id()?
                    || stamp.token_id != token
                    || stamp.claim != claim
                    || stamp.commitment != commitment
                    || stamp.expires_at != self.epoch.expires_at
                {
                    return Err(Error::Storage);
                }
                crate::admission::signature(
                    &self.epoch.redemption_public_key,
                    &stamp.signing_bytes()?,
                    &stamp.signature,
                )
            ...
            None => {
                let mut stamp = RedemptionStamp {
                    context_id: self.epoch.context_id()?,
                    token_id: token.clone(),
                    commitment: request.commitment.clone(),
                    claim: request.claim.clone(),
                    expires_at: self.epoch.expires_at,
                    signature: String::new(),
                };
                stamp.signature =
                    BASE64URL_NOPAD.encode(&self.key.sign(&stamp.signing_bytes()?).to_bytes());

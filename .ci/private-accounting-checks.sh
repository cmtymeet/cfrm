...
case "${CHECK_PHASE:-full}" in
  ...
  ledger)
    ...
    [[ "${REUSE_ARTIFACT_SHA256:-}" =~ ^[0-9a-f]{64}$ ]] || exit 2
    [[ "${BROWSER_BUNDLE_SHA:-}" =~ ^[0-9a-f]{40}$ ]] || exit 2
    case "${ACCOUNT_SCENARIOS:-}" in answer|close) ;; *) printf 'One explicit replay scenario required\n'; exit 2 ;; esac
...
if test "${CHECK_PHASE:-full}" = ledger; then
  ...
  (cd "$REUSE_ARTIFACT_DIR"; sha256sum --check --strict SHA256SUMS > "$artifact_dir/reuse-validation.log")
  cmp package-lock.json "$REUSE_ARTIFACT_DIR/package-lock.json"
  cmp ../../runtime/accounting/package-lock.json "$REUSE_ARTIFACT_DIR/runtime-package-lock.json"
  ...
  export ACCOUNTING_ARTIFACT_DIR="$artifact_dir" REUSE_ARTIFACT_DIR REUSE_ARTIFACT_SHA256 BROWSER_BUNDLE_SHA ACCOUNT_SCENARIOS
...
JS
  ...
  for scenario in ${ACCOUNT_SCENARIOS:-answer close}; do
    ...
    # Includes the 1200s browser bound, then independent Rust ledger verification
    # and child cleanup after Chromium has stopped.
    timeout --kill-after=15 1800 npm test 2>&1 | tee "$ACCOUNTING_ARTIFACT_DIR/browser-harness.log"

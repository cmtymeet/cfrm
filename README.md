...
The earlier certified onion rendezvous library and numerical experiments remain available separately. cfrm composes with externally operated eligibility verification and [cmsg](https://github.com/corbet-libs/cmsg) for private information transport. Eligibility policy and numerical disapproval are separate integration concerns. This repository does not provide a complete deployed community service.

The experimental [private account ledger](docs/account-ledger.md) adds durable opaque-state updates, lifetime genesis uniqueness and authenticated recovery across devices. The [v2 reciprocity policy](docs/reciprocity-policy.md) defines live introductions, shared bidirectional capacity, newcomer limits, irreversible admission turns and bounded refill. Private peer proofs connect accepted Active reservations to cmsg's first-payload gate. [Operational tuning](docs/tuning.md) changes the waiting period prospectively while preserving existing deadlines. The host supplies the complete pinned verifier and trusted storage. The [account-state evidence](experiments/private-accounting/account-state/README.md) records passing v2 Answer/Close browser proofs, real Rust ledger acceptance and protected cmsg release, with explicit host and recovery limits; mobile readiness remains unverified.

[Product service integration](docs/product-services.md) connects the signed discovery API, expiring encrypted profile cache, per-device presence, browser key access and reusable accounting runtime. Discovery uses a Valkey ciphertext cache with separate durable quota/replay guards. Browser profile keys stay with members; independently budgeted [blind key-access tickets](docs/key-access-tickets.md) protect key requests. The [accounting runtime](runtime/accounting/README.md) extracts proving, verification and authenticated client operations for application embedding.

The current library checkpoint at `9637a9f` passed [101 native tests, 75 JavaScript tests, Wasm checking and 63 Chromium contracts](https://github.com/corbet-libs/cfrm/actions/runs/35730678068). Its separate runtime suite passed 14 recovery/refresh and client/runtime tests plus a locked clean-consumer import of the actual Noir/BB dependencies. At `4a0f8ed`, the [same 63 browser contracts passed against the extracted npm archive](https://github.com/corbet-libs/cfrm/actions/runs/35735067948). The root JavaScript package and [Wasm companion package](browser/CONSUMER.md) are locally installable CI artifacts; neither was published to a registry.

The [real-proof run at `ee566e0`](https://github.com/corbet-libs/cfrm/actions/runs/35731679245) completed Answer, including the shipped runtime factory and verifier. Close generated and accepted all twelve scenario proofs, then timed out during the final generic factory demonstration; that browser run remains failed. At `09ba9af`, the retained Close proofs passed independent verification and a fresh Rust ledger replay, and the shipped verifier accepted twelve Close proofs plus the separately identified Answer factory proof with 26 corruption/scope rejections. This continuation generated no proofs and does not claim a completed Close browser run. Roster/time fixtures remain synthetic, and wallet persistence and real eligibility integration remain application responsibilities.

The core/browser CI runs on GHA, with Crow as the fallback. Earlier, at `3bc89d0`, 63 actual Chromium checks passed for the roster and blind permits, alongside 21 Rust and 55 historical JavaScript tests and portable Wasm checking. That older run covers roster/permit behavior. Crow9/58 at `eddc9283` separately passed the v2 Answer/Close integration: 224/229 browser checks, 22 account proofs and four peer proofs, with independent Node verification and real Rust ledger checks.

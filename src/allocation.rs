use crate::{
    admission::{
        decode, digest, signature, verify_admission, verify_device_authorization, AdmissionGrant,
...
pub fn policy_digest(policy: &AllocationPolicy) -> Result<String, Error> {
    if policy.period_seconds == 0
        || policy.initial_credits > policy.credit_cap
        || policy.periodic_credits > policy.credit_cap
        || policy.credit_cap > MAX_INTEGER
        || policy.period_seconds > MAX_INTEGER
        || policy.max_authorization_seconds == 0
        || policy.max_authorization_seconds > MAX_INTEGER
        || policy.max_request_bytes == 0
        || policy.max_request_bytes > usize::MAX / 2
    {
        return Err(Error::InvalidInput);
    }
    let bytes = serde_json::to_vec(&serde_json::json!(["cfrm.allocation.policy.v1", policy]))
...
pub fn allocation_bytes(request: &AllocationRequest) -> Result<Vec<u8>, Error> {
    ...
    let blinded = BASE64URL_NOPAD
        ...
        .map_err(|_| Error::InvalidInput)?;
    if blinded.is_empty() || BASE64URL_NOPAD.encode(&blinded) != request.blinded_request {
        return Err(Error::InvalidInput);
    }
...
impl AllocationLedger {
    pub fn open(
        ...
        let prior: Option<String> = transaction
            .query_row(
                "SELECT config FROM cfrm_allocation_config WHERE singleton=1",
                [],
                |row| row.get(0),
            )
            .optional()?;
        if let Some(prior) = prior {
            ...
            transaction.execute(
                "INSERT INTO cfrm_allocation_config(singleton,config,clock_floor) VALUES(1,?1,0)",
                [&config],
            )?;
        }
        transaction.commit()?;
    ...
    pub fn reserve(
        ...
        self.issue_with(grant, authorization, request, clock, |_, _, _| {
            Ok(Vec::new())
        })
    ...
    pub(crate) fn issue_with(
        ...
        request: &AllocationRequest,
        clock: impl Fn() -> u64,
        issue: impl FnOnce(&rusqlite::Transaction<'_>, &[u8], u64) -> Result<Vec<u8>, Error>,
    ) -> Result<(Reservation, Vec<u8>), Error> {
        let before = clock();
        verify_admission(grant, &self.trust, before)?;
        verify_device_authorization(authorization, grant, before)?;
        if request.community_id != grant.community_id
            || request.member_id != grant.member_id
            || request.chat_public_key != grant.chat_public_key
            || request.policy_digest != self.policy_digest
        {
            return Err(Error::Admission);
        }
        if request.blinded_request.len() > self.policy.max_request_bytes * 2 {
            return Err(Error::InvalidInput);
        }
        ...
        let floor = stored_integer(transaction.query_row(
            "SELECT clock_floor FROM cfrm_allocation_config WHERE singleton=1",
            [],
            |row| row.get::<_, i64>(0),
        )?)?;
        if now < floor || now < before || now > MAX_INTEGER {
        ...
        if let Some((prior, remaining_credits)) = prior {
            ...
            if remaining_credits > self.policy.credit_cap {
                return Err(Error::Storage);
            }
            ...
            let response: Option<Vec<u8>> = transaction
                .query_row(
                    "SELECT response FROM cfrm_allocation_responses WHERE member=?1 AND nonce=?2",
                    params![request.member_id, request.nonce],
                    |row| row.get(0),
                )
                .optional()?;
            transaction.execute(
                "UPDATE cfrm_allocation_config SET clock_floor=?1 WHERE singleton=1",
                [sql_integer(now)?],
            )?;
            transaction.commit()?;
            return Ok((
        ...
        let account: Option<(i64, i64)> = transaction
            .query_row(
                "SELECT credits,last_period FROM cfrm_allocation_members WHERE member=?1",
                [&request.member_id],
        ...
        let credits = match account {
            Some((credits, last_period)) => {
                ...
                if credits > self.policy.credit_cap {
                    return Err(Error::Storage);
                }
                let elapsed = period
                    .checked_sub(last_period)
                    .ok_or(Error::ClockRollback)?;
                credits
                    .saturating_add(elapsed.saturating_mul(self.policy.periodic_credits))
                    .min(self.policy.credit_cap)
            }
        ...
        if completed_at < now || completed_at > MAX_INTEGER {
            return Err(Error::ClockRollback);
        }
        if completed_at >= request.expires_at
        ...
        transaction.execute(
            "INSERT INTO cfrm_allocation_responses(member,nonce,response) VALUES(?1,?2,?3)",
            params![request.member_id, request.nonce, response],
        )?;
        transaction.execute(
            "UPDATE cfrm_allocation_config SET clock_floor=?1 WHERE singleton=1",
            [sql_integer(completed_at)?],
        )?;
        transaction.commit()?;
        Ok((
            Reservation {
    ...
    pub fn balance(&self, member_id: &str) -> Result<Option<u64>, Error> {
        ...
        let balance: Option<i64> = self
            .connection
            .query_row(
                "SELECT credits FROM cfrm_allocation_members WHERE member=?1",
                [member_id],
                |row| row.get(0),
            )
            .optional()?;
        balance
            .map(|value| {
                let value = stored_integer(value)?;
                if value > self.policy.credit_cap {
                    return Err(Error::Storage);
                }
                Ok(value)
            })
    ...
    pub fn resolve_private(&mut self, _proof: &[u8]) -> Result<(), Error> {

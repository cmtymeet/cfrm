...
use crate::{discovery::{DiscoveryService, DiscoveryStore}, Error};
use axum::{body::to_bytes, extract::{Request, State}, http::{header, StatusCode},
...
pub fn router<S: DiscoveryStore + 'static>(service: Arc<DiscoveryService<S>>,
    clock: Arc<dyn Fn() -> u64 + Send + Sync>, limits: DiscoveryHttpLimits) -> Result<Router, Error> {
    if limits.max_concurrent_requests == 0 || limits.max_concurrent_requests > Semaphore::MAX_PERMITS
        || limits.body_timeout_millis == 0 {
        return Err(Error::InvalidInput);
    }
    let allowed_hosts = limits.allowed_hosts.map(|hosts| {
        if hosts.is_empty() || hosts.len() > 256 { return Err(Error::InvalidInput); }
        hosts.iter().map(|host| normalize_host(host).ok_or(Error::InvalidInput)).collect::<Result<Vec<_>, _>>()
    }).transpose()?;
    let state = Arc::new(ApiState { service, clock, allowed_hosts,
    ...
    Ok(Router::new().route("/v1/discovery", post(handle::<S>)).with_state(state))
}

fn normalize_host(value: &str) -> Option<String> {
    if value.is_empty() || value.len() > 259 || !value.is_ascii() { return None; }
    ...
    if expected != value { return None; }
    Some(value.to_ascii_lowercase())
}

fn rejected(status: StatusCode) -> Response {
    (status, [(header::CONTENT_TYPE, "application/json"), (header::CACHE_CONTROL, "no-store")],
        "{\"error\":\"discovery request rejected\"}").into_response()
}

async fn handle<S: DiscoveryStore + 'static>(State(state): State<Arc<ApiState<S>>>, request: Request) -> Response {
    ...
    if let Some(allowed) = &state.allowed_hosts {
        ...
        let actual = hosts.next().and_then(|v| v.to_str().ok()).and_then(normalize_host);
        if hosts.next().is_some() || actual.as_ref().is_none_or(|host| !allowed.contains(host)) {
            return rejected(StatusCode::MISDIRECTED_REQUEST);
        }
    }
    if request.headers().get(header::CONTENT_TYPE).and_then(|v| v.to_str().ok())
        .and_then(|v| v.split(';').next()).map(str::trim) != Some("application/json") {
        return rejected(StatusCode::UNSUPPORTED_MEDIA_TYPE);
    }
    let input = match tokio::time::timeout(state.body_timeout,
        to_bytes(request.into_body(), state.service.limits().max_request_bytes)).await {
        Ok(Ok(input)) => input,
    ...
    match completed {
        Ok(Ok(result)) => match serde_json::to_vec(&result) {
            Ok(bytes) => (StatusCode::OK,
                [(header::CONTENT_TYPE, "application/json"), (header::CACHE_CONTROL, "no-store")], bytes).into_response(),
            Err(_) => rejected(StatusCode::SERVICE_UNAVAILABLE),
        },
        Ok(Err(Error::Capacity)) => rejected(StatusCode::TOO_MANY_REQUESTS),
        Ok(Err(Error::Storage | Error::ClockRollback)) | Err(_) => rejected(StatusCode::SERVICE_UNAVAILABLE),
...
mod tests {
    ...
    fn host_allowlist_normalizes_ascii_case_and_rejects_ambiguous_authority() {
        assert_eq!(normalize_host("API.Example.TEST:8443"), Some("api.example.test:8443".into()));
        for invalid in ["", "api.example.test.", "a..test", "-a.test", "a-.test",
            "user@api.example.test", "api.example.test/path", "api.example.test:0",
            "api.example.test:70000", " api.example.test", "api.exämple.test", "*.example.test"] {

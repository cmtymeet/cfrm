...
fn request(f: &Fixture, key: &SigningKey, nonce: u8, now: u64) -> AllocationRequest {
    let mut request = AllocationRequest {
        ...
        nonce: encoded(nonce),
        blinded_request: BASE64URL_NOPAD.encode(&[nonce; 384]),
        issued_at: now,
        expires_at: now + 40,
        signature: String::new(),
    };
    let hash = BASE64URL_NOPAD.encode(&Sha256::digest(
        BASE64URL_NOPAD
...
fn forged_or_expired_request_leaves_no_account_and_nonce_cannot_change_content() {
    ...
    let mut forged = request(&f, &f.device, 10, 110);
    forged.blinded_request = BASE64URL_NOPAD.encode(&[11; 384]);
    assert_eq!(
        ledger.reserve(&grant, &f.authorize(5, &f.device), &forged, || 110),
        Err(Error::Signature)
    );
    assert_eq!(ledger.balance(&member_id(5)).unwrap(), None);
    let good = request(&f, &f.device, 10, 110);
    assert_eq!(
        ledger.reserve(&grant, &f.authorize(5, &f.device), &good, || 150),
        Err(Error::Expired)
    );
    ledger
        .reserve(&grant, &f.authorize(5, &f.device), &good, || 110)
        .unwrap();
    let changed = request(&f, &f.device, 10, 111);
    assert_eq!(
        ledger.reserve(&grant, &f.authorize(5, &f.device), &changed, || 111),
        Err(Error::Replay)
    );
...
fn periodic_allowance_cannot_be_reclaimed_on_reconnect_or_clock_rollback() {
    ...
    changed.initial_credits = 3;
    assert!(matches!(
        AllocationLedger::open(&path, f.trust.clone(), changed),
...
fn resolve_answer_or_close_fails_closed_without_proof_backend() {
    let f = Fixture::new();
    let mut l = AllocationLedger::open(":memory:", f.trust.clone(), policy()).unwrap();
    l.reserve(
        &f.grant(5, &f.device),
        &f.authorize(5, &f.device),
        &request(&f, &f.device, 10, 110),
    ...
    for forged in [
        b"answered".as_slice(),
        b"closedForever".as_slice(),
        b"".as_slice(),
    ] {
        assert_eq!(l.resolve_private(forged), Err(Error::UnsupportedCapability));
    }
    assert_eq!(l.balance(&member_id(5)).unwrap(), Some(1));
}
...
fn simultaneous_devices_cannot_exceed_identity_allowance() {
    ...
    for seed in [10, 11, 12] {
        ...
        workers.push(std::thread::spawn(move || {
            ...
            let grant = f.grant(5, &key);
            let authorization = f.authorize(5, &key);
            let request = request(&f, &key, seed, 110);
            ready.wait();
            ledger.reserve(&grant, &authorization, &request, || 110)
        }));
    }

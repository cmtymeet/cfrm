...
use crate::{
    accounting::{AccountAcceptance, AccountPolicy, AccountProofScope, AccountProofVerifier,
        AccountRequest, AccountStatement, AccountStatusRequest, AccountStatusResponse},
    accounting_ledger::{AccountLedger, WaitingPeriodTuning},
    admission::{AdmissionGrant, DeviceAuthorization}, Error,
};
use serde::{Deserialize, Serialize};
use std::{io::{Read, Write}, path::PathBuf, process::{Command, Stdio},
    sync::{Arc, atomic::{AtomicUsize, Ordering}}, thread, time::{Duration, Instant}};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(tag = "action", rename_all = "camelCase", deny_unknown_fields)]
pub enum AccountServiceRequest {
    Apply { grant: AdmissionGrant, authorization: DeviceAuthorization, request: AccountRequest },
    Status { grant: AdmissionGrant, authorization: DeviceAuthorization, request: AccountStatusRequest },
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(tag = "action", content = "value", rename_all = "camelCase", deny_unknown_fields)]
...
impl<V: AccountProofVerifier, C: Fn() -> u64> AccountService<V, C> {
    pub fn new(ledger: AccountLedger<V>, clock: C, max_request_bytes: usize) -> Result<Self, Error> {
        if max_request_bytes == 0 || max_request_bytes > 16 * 1024 * 1024 {
            return Err(Error::InvalidInput);
        }
        Ok(Self { ledger, clock, max_request_bytes })
    }

    /// Enforce this bound while reading the transport body too, before allocation.
    pub fn max_request_bytes(&self) -> usize { self.max_request_bytes }
    ...
    pub fn handle(&mut self, request: AccountServiceRequest) -> Result<AccountServiceResponse, Error> {
        ...
        match request {
            AccountServiceRequest::Apply { grant, authorization, request } =>
                self.ledger.apply(&grant, &authorization, &request, &self.clock).map(AccountServiceResponse::Apply),
            AccountServiceRequest::Status { grant, authorization, request } =>
                self.ledger.status(&grant, &authorization, &request, &self.clock).map(AccountServiceResponse::Status),
        }
    }
    ...
    pub fn tune_waiting_period(&mut self, expected_revision: u64, seconds: u64) -> Result<WaitingPeriodTuning, Error> {
        self.ledger.reload_policy()?;
        self.ledger.update_waiting_period(expected_revision, seconds, &self.clock)
    }
}
...
impl ProcessAccountVerifier {
    pub fn new(config: ProcessVerifierConfig) -> Result<Self, Error> {
        if !config.node.is_absolute() || !config.script.is_absolute() || !config.artifact_config.is_absolute()
            || config.scope.circuit_digest == [0; 32] || config.scope.verifying_key_digest == [0; 32]
        ...
        Ok(Self { config: Arc::new(config), active: Arc::new(AtomicUsize::new(0)) })
    }
}

struct ActivePermit(Arc<AtomicUsize>);
impl Drop for ActivePermit { fn drop(&mut self) { self.0.fetch_sub(1, Ordering::AcqRel); } }

impl AccountProofVerifier for ProcessAccountVerifier {
    fn scope(&self) -> AccountProofScope { self.config.scope.clone() }

    fn verify(&self, statement: &AccountStatement, proof: &[u8]) -> Result<(), Error> {
        if proof.is_empty() || proof.len() > self.config.max_proof_bytes { return Err(Error::InvalidInput); }
        self.active.fetch_update(Ordering::AcqRel, Ordering::Acquire, |n|
            (n < self.config.maximum_parallel).then_some(n + 1)).map_err(|_| Error::Capacity)?;
        ...
        let mut child = Command::new(&self.config.node)
            .arg(format!("--max-old-space-size={}", self.config.node_heap_megabytes))
            .arg(&self.config.script).arg(&self.config.artifact_config)
        ...
        let reader = thread::spawn(move || {
            let mut bytes = Vec::new(); stdout.take(4097).read_to_end(&mut bytes).map(|_| bytes)
        });
        ...
        let output = reader.join().map_err(|_| Error::CryptoProvider)?.map_err(|_| Error::CryptoProvider)?;
        if !wrote || !status.is_some_and(|v| v.success()) || output.len() > 4096 { return Err(Error::CryptoProvider); }
        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase", deny_unknown_fields)]
        struct Verdict { verified: bool, proof_scope: AccountProofScope }
        let verdict: Verdict = serde_json::from_slice(&output).map_err(|_| Error::CryptoProvider)?;
        if !verdict.verified || verdict.proof_scope != self.config.scope { return Err(Error::CryptoProvider); }
        Ok(())
    }
}
